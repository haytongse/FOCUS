...
import { updateCampusApi, ApiCampus } from '../../services/focusApi';
...
const EditCampusScreen: React.FC<Props> = ({ campus, onBack, onSaved }) => {
  const [nameEn, setNameEn] = useState(campus.nameEn ?? '');
  const [nameKm, setNameKm] = useState(campus.nameKm ?? '');
  const [code, setCode] = useState(campus.campusCode ?? '');
  const [address, setAddress] = useState(campus.address ?? '');
  const [active, setActive] = useState(campus.active !== false);
  ...
  const handleSave = () => {
    ...
    showAlert({
      type: 'confirm',
      title: 'Save Changes',
      message: `Save changes to campus "${code.trim()}" — ${nameEn.trim()}?`,
      actions: [
        { label: 'Cancel', variant: 'outline' },
        {
          ...
          onPress: async () => {
            ...
            setLoading(true);
            try {
              await updateCampusApi(String(campus.id), {
                nameEn: nameEn.trim(),
                nameKm: nameKm.trim(),
              ...
              setApiError(err?.message ?? 'Failed to update campus');
            } finally {
              setLoading(false);
            }
          },
  ...
  return (
    <View style={styles.safe}>
      <AppBar
        title="Edit Campus"
        subtitle={code ? `Code: ${code}` : 'Edit campus details'}
        titleAlign="left"
        showBack
        onBack={onBack}
      />
